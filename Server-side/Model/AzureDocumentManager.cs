    ...
    public class AzureDocumentManager
    {
        ...
        public void RegisterAzure(string accountName, string accountKey, string blobName)
        {
            container = new BlobServiceClient(
                new Uri(blobPath.Substring(0, blobPath.Length - blobName.Length - 1)),
                new StorageSharedKeyCredential(accountName, accountKey),
                null
            ).GetBlobContainerClient(blobName);
        }
        ...
        protected async Task<FileManagerResponse> GetFilesAsync(string path, string filter, FileManagerDirectoryContent[] selectedItems)
        {
            ...
            try
            {
                var blobPages = container
                    .GetBlobsAsync(traits: BlobTraits.None, states: BlobStates.None, prefix: path, cancellationToken: CancellationToken.None)
                    .AsPages()
                    .GetAsyncEnumerator();
                ...
                string[] extensions = ((filter.Replace(" ", "")) ?? "*")
                ...
                if (directoryExists)
                {
                    foreach (Azure.Page<BlobHierarchyItem> page in container
                        .GetBlobsByHierarchy(traits: BlobTraits.None, states: BlobStates.None, prefix: path, delimiter: "/", cancellationToken: CancellationToken.None)
                        .AsPages())
                    {
                        foreach (BlobItem item in page.Values.Where(i => i.IsBlob).Select(i => i.Blob))
                        {
                            ...
                            if (!(extensions[0].Equals("*.*") || extensions[0].Equals("*")))
                            {
                                if (!(Array.IndexOf(
                                        extensions,
                                        "*." + (item.Name.ToString().Trim().Split('.'))[item.Name.ToString().Trim().Split('.').Length - 1]
                                    ) >= 0))
                                {
                                    includeItem = false;
                                }
                            ...
                            if (includeItem)
                            {
                                ...
                                entry.IsFile = true;
                                entry.Size = item.Properties.ContentLength ?? 0L;
                                entry.DateModified = item.Properties.LastModified?.LocalDateTime ?? DateTime.MinValue;
                        ...
                        foreach (string item in page.Values.Where(i => i.IsPrefix).Select(i => i.Prefix))
                        {
                            FileManagerDirectoryContent entry = new FileManagerDirectoryContent();
                        ...
                        prefixes = page.Values.Where(i => i.IsPrefix).Select(i => i.Prefix).ToList();
                    }
                }
            ...
            try
            {
                ...
                {
                    readResponse.Files = new List<FileManagerDirectoryContent>();
        ...
        {
            foreach (Azure.Page<BlobItem> page in container
                .GetBlobs(traits: BlobTraits.None, states: BlobStates.None, prefix: path, cancellationToken: CancellationToken.None)
                .AsPages())
            {
                var item = page.Values.ToList().OrderByDescending(m => m.Properties.LastModified).FirstOrDefault();
                var lastModified = item?.Properties?.LastModified;
                if (lastModified.HasValue)
                {
                    DateTime checkFileModified = lastModified.Value.LocalDateTime;
                    ...
                    foreach (FileManagerDirectoryContent fileItem in selectedItems)
                    {
                        if (names.Length == 1)
                        {
                            if (fileItem.IsFile)
                            {
                                ...
                                fileDetails.Size = ByteConversion(properties.ContentLength);
                                fileDetails.Modified = properties.LastModified.LocalDateTime;
            ...
            try
            {
                ...
                foreach (FileManagerDirectoryContent fileItem in selectedItems)
                {
                    ...
                    else
                    {
                        ...
                        foreach (Azure.Page<BlobItem> page in container
                            .GetBlobs(traits: BlobTraits.None, states: BlobStates.None, prefix: path + fileItem.Name + "/", cancellationToken: CancellationToken.None)
                            .AsPages())
                        {
                            foreach (BlobItem item in page.Values)
                            {
        ...
        private async Task DownloadFolder(string path, string folderName, ZipArchiveEntry? zipEntry, ZipArchive archive)
        {
            ...
            foreach (Azure.Page<BlobHierarchyItem> page in container
                .GetBlobsByHierarchy(traits: BlobTraits.None, states: BlobStates.None, prefix: pathValue, delimiter: "/", cancellationToken: CancellationToken.None)
                .AsPages())
            {
                foreach (BlobItem item in page.Values.Where(i => i.IsBlob).Select(i => i.Blob))
                {
                ...
                foreach (string item in page.Values.Where(i => i.IsPrefix).Select(i => i.Prefix))
                {
                    string absoluteFilePath = item.Replace(blobPath, "");
                    pathValue = absoluteFilePath;

                    string targetPath = item.Replace(filesPath + "/", "");
                    string folderPath = new DirectoryInfo(targetPath).Name;

                    currentFolderName = previousFolderName.Length > 1
                        ? item.Replace(previousFolderName, "").Trim('/')
                        : item.Trim('/');
        ...
        private async Task<bool> IsFolderExists(string path)
        {
            ...
            foreach (Azure.Page<BlobHierarchyItem> page in container
                ...
                .AsPages())
            {
                x = page.Values.Where(i => i.IsPrefix).Select(i => i.Prefix).ToList();
            }
            return x.Count > 0;
        }
        ...
        private void CopySubFolder(FileManagerDirectoryContent subFolder, string targetPath)
        {
            ...
            foreach (Azure.Page<BlobHierarchyItem> page in container
                .GetBlobsByHierarchy(traits: BlobTraits.None, states: BlobStates.None, prefix: subFolder.Path + "/", delimiter: "/", cancellationToken: CancellationToken.None)
                .AsPages())
            {
                foreach (BlobItem item in page.Values.Where(i => i.IsBlob).Select(i => i.Blob))
                {
        ...
        private async Task<string> FileRename(string newPath, string fileName)
        {
            ...
            while (index > -1
                ? await IsFileExists(newPath + nameNotExist + (fileCount > 0 ? "(" + fileCount.ToString() + ")" + Path.GetExtension(fileName) : Path.GetExtension(fileName)))
                : await IsFolderExists(newPath + nameNotExist + (fileCount > 0 ? "(" + fileCount.ToString() + ")" + Path.GetExtension(fileName) : Path.GetExtension(fileName))))
            {
                fileCount++;
            }
